...
currentFileName = 'GraphFusionModelV12_7'
...
valPredictionList = np.array(tmpPreList, dtype=np.float32)

n2 = np.mean((valPredictionList - np.dot(valTarget, demandMaskTensorFeed).reshape([-1, 1]))**2)

# compute the uncertainty
finalPreResult = np.mean(mulPredictionList, axis=0)

n1 = np.var(mulPredictionList, axis=0)

# np.var(mulPredictionList, axis=0)+
uncertainty = np.sqrt(n1 + n2)
