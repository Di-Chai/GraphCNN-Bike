...
if __name__ == '__main__':
    ...
    # demand graph
    transitionMatrix = np.loadtxt(os.path.join(txtPath, 'transitionMatrix-2016-01-01-2017-01-01.txt'), delimiter=' ')
    ...
    np.savetxt(os.path.join(txtPath, 'demandGraphMatrix-2016-01-01-2017-01-01.txt'), demandGraphMatrix, newline='\n', delimiter=' ')
    ...
    if os.path.isfile(os.path.join(txtPath, 'demandListForGraph.txt')) is False:
        ...
        for stationIndex in range(len(allStationIDList)):
            ...
            stationMinDemandData = getJsonDataFromPath(
                os.path.join(demandMinDataPath, allStationIDList[stationIndex] + '.json'))
            dayIn = []
            dayOut = []
            daySum = []
            while date <= endData:
