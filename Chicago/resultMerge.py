...
codeVersion = 'GraphFusionModelV9'
...
finalPreResultFileList = [txtFileName[e] for e in range(len(txtFileName)) if e % 3 == 0]
testTargetFileList = [txtFileName[e] for e in range(len(txtFileName)) if e % 3 == 1]
...
targetFolder = GraphDemandPreDataPath
txtFileName = [e for e in os.listdir(targetFolder) if e.endswith('.txt') and e.startswith(codeVersion) and\
               'GraphSingleStationDemandPreV10' not in e and 'GraphSingleStationDemandPreV11' not in e]
txtFileRankList = [int(e.split('-')[1]) for e in txtFileName]
txtFileName = [e[1] for e in sorted(zip(txtFileRankList, txtFileName), key = lambda x: x[0])]
