...
Z = 1.96  # a = 95%
def drawSingleStation(myRank, n_jobs, savePath, finalPre, testTarget, plotFlag, stationNumber, codeVersion):
...
if __name__ == '__main__':
    codeVersion = 'GraphFusionModelV99'
    ...
    for i in range(0, txtFileName.__len__(), fileGap):
        ...
        finalPreResult = np.loadtxt(os.path.join(targetFolder, txtFileName[i]), delimiter=' ').reshape([-1, 24-6])[0:plotDayLength, :]
        testTarget = np.loadtxt(os.path.join(targetFolder, txtFileName[i+1]), delimiter=' ').reshape([-1, 24-6])[0:plotDayLength, :]

        p = Pool()
        for rank in range(n_jobs):
            p.apply_async(drawSingleStation,
                          args=(rank, n_jobs, savePath, finalPreResult, testTarget, False, stationNumber, codeVersion), )
        p.close()
        p.join()
    ...
    np.savetxt(os.path.join(txtPath, codeVersion + '_%s' % plotDayLength + '.txt'), np.array(singleStationRMSEList, dtype=np.float32), delimiter=' ',
               newline='\n')
