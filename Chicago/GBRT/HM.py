...
for i in range(predictDayLength):
    ...
    predictionResult.append(np.mean(candidateList, axis=0)[:(24-6), :])

predictionResult = np.array(predictionResult, dtype=np.float32)
corrTestTarget = testTarget[:predictDayLength, :(24-6), :]
...
for i in range(targetStationNumbers):
    ...
    RMSE = np.mean((corrTestTarget[:, :, stationIndex] - predictionResult[:, :, stationIndex])**2)**0.5
