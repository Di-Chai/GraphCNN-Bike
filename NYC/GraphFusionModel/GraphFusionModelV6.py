...
# currentFileName = 'GraphFusionModelV5_0'
...
with autoEncoderGraph.as_default():
    ...
    with tf.variable_scope('encoder_fusionGraph'):
        ...
        convDemandGraph = tf.reshape(
            tf.matmul(tf.reshape(encoderRawInput, [-1, stationNumber]), demandGraphMatrixTensor),
            [-1, n_steps_encoder, 1])
        convDistGraph = tf.reshape(
            tf.matmul(tf.reshape(encoderRawInput, [-1, stationNumber]), distGraphMatrixTensor),
            [-1, n_steps_encoder, 1])

        inputWeightDistGraph = tf.Variable(tf.random_normal([n_inputs, n_inputs]))
        inputWeightDemandGraph = tf.Variable(tf.random_normal([n_inputs, n_inputs]))
        inputBias = tf.Variable(tf.constant(0.1, shape=[n_inputs, ]))

        encoderInput = tf.matmul(tf.reshape(convDemandGraph, [-1, n_inputs]), inputWeightDemandGraph)\
                       + tf.matmul(tf.reshape(convDistGraph, [-1, n_inputs]), inputWeightDistGraph) + inputBias
...
if trainAutoEncoder:
    ...
    for epoch in range(finishedEpoch, n_epoch):
        ...
        for iteration in range(currentIterations):
            ...
            batch_decoder_input = testFeature[pointer0:pointer1][:, -n_steps_decoder:, :].reshape(
                [batch_size, n_steps_decoder, stationNumber])
            batch_decoder_target = testTarget[pointer0: pointer1].reshape([batch_size, stationNumber])
            testLoss = autoEncoderSession.run(loss_autoEncoder,
                                                  feed_dict={
                                                      demandMaskTensorAutoEncoder: demandMaskTensorFeed,
                                                      demandGraphMatrixTensorRaw: demandGraphMatrixFeed,
                                                      distGraphMatrixTensorRaw: distanceGraphMatrixFeed,
        ...
        print('%s Training loss_autoEncoder after %s epoch : %s, Test loss_autoEncoder : %s' %
              (codeVersion, epoch, np.mean(trainLossList), np.mean(testLossList)))
        autoEncoderLossListEpoch.append(np.mean(trainLossList))
        autoEncoderTestLossListEpoch.append(np.mean(testLossList))
