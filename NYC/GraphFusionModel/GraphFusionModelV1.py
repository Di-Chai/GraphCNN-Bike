...
n_units_pre = [n_hidden_units*2 + addFeatureLength * 5 + 5, n_hidden_units*2, n_hidden_units, n_hidden_units/2, predictLength]
...
with autoEncoderGraph.as_default():
    ...
    # Demand Graph
    with tf.variable_scope('encoderDemandGraph'):
        demandGraphMatrixTensorRaw = tf.placeholder(tf.float32, [stationNumber, 1])
        demandGraphMatrixWeight = tf.matrix_diag(tf.Variable(tf.random_normal([stationNumber])))
        demandGraphMatrixTensor = tf.matmul(demandGraphMatrixWeight, demandGraphMatrixTensorRaw)

        encoderCell_DemandGraph = tf.nn.rnn_cell.LSTMCell(n_hidden_units, state_is_tuple=True)
        encoderCell_DemandGraph = tf.nn.rnn_cell.DropoutWrapper(encoderCell_DemandGraph,
                                                                output_keep_prob=1 - dropout_pro)
        encoderInputDemandGraph = tf.reshape(
            tf.matmul(tf.reshape(encoderRawInput, [-1, stationNumber]), demandGraphMatrixTensor),
            [-1, n_steps_encoder, 1])

        en_outputs_DemandGraph, en_final_state_DemandGraph = tf.nn.dynamic_rnn(encoderCell_DemandGraph, encoderInputDemandGraph,
                                                                               dtype=tf.float32,
                                                                               time_major=False)

    # Dist Graph
    with tf.variable_scope('encoderDistGraph'):
        distGraphMatrixTensorRaw = tf.placeholder(tf.float32, [stationNumber, 1])
        distGraphMatrixWeight = tf.matrix_diag(tf.Variable(tf.random_normal([stationNumber])))
        distGraphMatrixTensor = tf.matmul(distGraphMatrixWeight, distGraphMatrixTensorRaw)

        encoderCell_DistGraph = tf.nn.rnn_cell.LSTMCell(n_hidden_units, state_is_tuple=True)
        encoderCell_DistGraph = tf.nn.rnn_cell.DropoutWrapper(encoderCell_DistGraph, output_keep_prob=1 - dropout_pro)

        encoderInputDistGraph = tf.reshape(
            tf.matmul(tf.reshape(encoderRawInput, [-1, stationNumber]), distGraphMatrixTensor),
            [-1, n_steps_encoder, 1])

        en_outputs_DistGraph, en_final_state_DistGraph = tf.nn.dynamic_rnn(encoderCell_DistGraph, encoderInputDistGraph,
                                                                           dtype=tf.float32,
                                                                           time_major=False)

    # fusion part
    fusionC = tf.concat([en_final_state_DemandGraph[0], en_final_state_DistGraph[0]], axis=1)
    fusionH = tf.concat([en_final_state_DemandGraph[1], en_final_state_DistGraph[1]], axis=1)

    fusionState = tf.contrib.rnn.LSTMStateTuple(fusionC, fusionH)
    ...
    with tf.variable_scope('decoder') as vs:
        ...
        decoderCell = tf.nn.rnn_cell.LSTMCell(n_hidden_units * 2, state_is_tuple=True)
        decoderCell = tf.nn.rnn_cell.DropoutWrapper(decoderCell, output_keep_prob=1 - dropout_pro)
        decoderOutputUnitNumber = n_hidden_units * 2
...
with preNNGraph.as_default():
    ...
    inputEmbeddingFeature = tf.placeholder(tf.float32, [None, n_hidden_units*2])
    ...
    inputFeature = tf.concat([inputEmbeddingFeature,
                              otherFeature, otherFeature, otherFeature, otherFeature, otherFeature,
        ...
        for iteration in range(currentIterations):
            ...
            trainLoss, _ = autoEncoderSession.run([loss_autoEncoder, trainOperation_autoEncoder],
                                    feed_dict={
                                        ...
                                        demandGraphMatrixTensorRaw: demandMaskTensorFeed,
