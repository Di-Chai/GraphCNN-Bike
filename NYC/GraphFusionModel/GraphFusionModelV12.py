...
# currentFileName = 'GraphFusionModelV11_6'
...
corrGraphMatrix = np.loadtxt(os.path.join(txtPath, 'correlationGraphMatrix.txt'), delimiter=' ')
...
demandGraphMatrixFeed = np.array(demandGraphMatrix[allStationIDList.index(centralStationIDList[targetStationIndex])]) \
                          .reshape([stationNumber, 1])
corrGraphMatrixFeed = np.array(corrGraphMatrix[allStationIDList.index(centralStationIDList[targetStationIndex])]) \
                          .reshape([stationNumber, 1])
###################################################################################################################
# Build The NetWork
#########################################################################
...
with autoEncoderGraph.as_default():
    ...
    with tf.variable_scope('encoder_fusionGraph'):
        ...
        # channel 3
        corrGraphMatrixTensorRaw = tf.placeholder(tf.float32, [stationNumber, 1])
        corrGraphMatrixWeight = tf.Variable(tf.random_normal([stationNumber, 1]))

        graphWeightMatrix = tf.concat([demandGraphMatrixWeight, distGraphMatrixWeight, corrGraphMatrixWeight], 1)
        ...
        corrGraphMatrixTensor = tf.multiply(tf.reshape(graphWeightMatrixSoftMax[:, 2], [-1, 1]), corrGraphMatrixTensorRaw)

        # fusion graph
        fusionGraphMatrixWeight = tf.Variable(tf.random_normal([stationNumber, 1]))
        fusionGraphMatrixTensor = tf.multiply((demandGraphMatrixTensor + distGraphMatrixTensor + corrGraphMatrixTensor),
                                              fusionGraphMatrixWeight)
        ...
        for iteration in range(currentIterations):
            ...
            trainLoss, _ = autoEncoderSession.run([loss_autoEncoder, trainOperation_autoEncoder],
                                    feed_dict={
                                        ...
                                        corrGraphMatrixTensorRaw: corrGraphMatrixFeed,
