...
# currentFileName = 'GraphFusionModelV7_0'
...
with autoEncoderGraph.as_default():
    ...
    with tf.variable_scope('encoder_fusionGraph'):
        ...
        demandGraphMatrixWeight = tf.constant(0.5, shape=[stationNumber, 1])
        demandGraphMatrixTensor = tf.multiply(demandGraphMatrixWeight, demandGraphMatrixTensorRaw)
        # channel 2
        distGraphMatrixTensorRaw = tf.placeholder(tf.float32, [stationNumber, 1])
        distGraphMatrixWeight = tf.constant(0.5, shape=[stationNumber, 1])
        distGraphMatrixTensor = tf.multiply(distGraphMatrixWeight, distGraphMatrixTensorRaw)
